...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        # Split the first word as the class name, the rest as parameters
        ...
        # Iterate over the remaining arguments as parameters
        for param in args_list[1:]:
            key, value = param.split('=')
            if '"' in value:  # Handle string with spaces and quotes
                value = value.replace('_', ' ').strip('"').replace('\\"', '"')
            elif '.' in value:  # Handle float
                try:
                    value = float(value)
                except ValueError:
